...
Standalone Python HTTP server that bridges SafeClaw to Android SMS via
...
3. Start the bridge:
   ```
   export WEBHOOK_URL="http://<SafeClaw-ip>:3030/api/messaging/incoming"
   python server.py
   ```
...
| `WEBHOOK_URL` | `http://127.0.0.1:3030/api/messaging/incoming` | SafeClaw incoming message endpoint |
...
**Outbound (SafeClaw -> SMS):** SafeClaw POSTs to the bridge's
`/send` endpoint with `{"to": "+1...", "text": "..."}`. The bridge calls
`termux-sms-send` to dispatch the message.

**Inbound (SMS -> SafeClaw):** A background thread polls
`termux-sms-list` every `POLL_INTERVAL` seconds, compares timestamps to
find new messages, and forwards them to SafeClaw's webhook as:
...
SafeClaw must be reachable from the phone. Common setups:
...
SafeClaw connects to the bridge at `http://<phone-ip>:3041/send`, so
...
| Webhook fails with connection refused | Verify `WEBHOOK_URL` points to a reachable SafeClaw instance |
