...
# safeclaw on macOS using Seatbelt profiles (sandbox-exec), a dedicated
# service user, and launchd.
#
# Usage:
#   sandbox-install-macos.sh setup   [-b /path/to/safeclaw]
#   sandbox-install-macos.sh start   [-- safeclaw-args...]
...
#   SA_ROOT      Sandbox root directory  (default: /usr/local/var/safeclaw)
#   SA_BINARY    Path to safeclaw bin  (default: ./target/release/safeclaw)
#   SA_USER      Runtime user            (default: _safeclaw)
#
...
SA_ROOT="${SA_ROOT:-/usr/local/var/safeclaw}"
SA_USER="${SA_USER:-_safeclaw}"
SA_BINARY="${SA_BINARY:-./target/release/safeclaw}"
PLIST_LABEL="com.pegasusheavyindustries.safeclaw"
PLIST_PATH="$HOME/Library/LaunchAgents/${PLIST_LABEL}.plist"
SANDBOX_PROFILE="$SA_ROOT/etc/sandbox.sb"
PID_FILE="$SA_ROOT/run/safeclaw.pid"
...
create_user() {
    ...
    dscl . -create "/Groups/$SA_USER" RealName "safeclaw service"
    ...
    dscl . -create "/Users/$SA_USER" RealName "safeclaw service account"
...
create_dirs() {
    ...
    # Ownership: safeclaw owns the whole tree
...
;; Seatbelt sandbox profile for safeclaw
...
install_binary() {
    ...
    install -m 0755 "$binary" "$SA_ROOT/bin/safeclaw"
    log "Installed safeclaw binary to $SA_ROOT/bin/"
}
...
<dict>
    ...
    <array>
        ...
        <string>${SA_ROOT}/bin/safeclaw</string>
    </array>
...
cmd_start() {
    ...
    local exe="$SA_ROOT/bin/safeclaw"
    if [ ! -f "$exe" ]; then
        err "safeclaw not found in sandbox. Run setup first."
        exit 1
    fi
    ...
    log "Starting safeclaw as $SA_USER with Seatbelt sandbox..."
...
cmd_stop() {
    ...
    pkill -u "$SA_USER" -f safeclaw 2>/dev/null || true
...
cmd_status() {
    ...
    printf '\033[0;36m── safeclaw sandbox status (macOS) ──\033[0m\n\n'
    ...
    if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
        local pid
        pid="$(cat "$PID_FILE")"
        printf '  \033[0;32m●\033[0m safeclaw running (PID %s)\n' "$pid"
    elif pgrep -u "$SA_USER" -f safeclaw &>/dev/null; then
        local pid
        pid="$(pgrep -u "$SA_USER" -f safeclaw | head -1)"
        printf '  \033[0;32m●\033[0m safeclaw running (PID %s)\n' "$pid"
    else
        printf '  \033[0;31m○\033[0m safeclaw not running\n'
    fi
...
usage() {
    cat <<EOF
safeclaw macOS sandbox installer

Usage: $(basename "$0") <command> [options]

Commands:
  setup    [-b binary]   Install dependencies, create user, build sandbox, install launchd plist
  start    [-- args...]  Start safeclaw in Seatbelt sandbox as service user
  stop                   Stop safeclaw
...
Examples:
  ...
  sudo ./scripts/sandbox-install-macos.sh setup -b /usr/local/bin/safeclaw
