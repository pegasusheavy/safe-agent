<script lang="ts">
    ...
    async function saveConfig() {
        ...
        try {
            ...
            const data = await res.json();
            if (!data.ok) {
                error = data.message || 'Failed to save config';
            }
        } catch (e) {
            error = 'Network error saving config';
        } finally {
            saving = false;
        }
    }
    ...
    async function testLlm() {
        ...
        try {
            ...
            llmTestOk = false;
            llmTestResult = 'Network error testing LLM connection';
        } finally {
            llmTesting = false;
        }
    }
    ...
    async function completeOnboarding() {
        ...
        try {
            ...
            error = 'Network error completing onboarding';
        } finally {
            completing = false;
        }
    }
        ...
        <div class="bg-surface rounded-xl border border-border shadow-2xl overflow-hidden">
            ...
            <div class="p-8">
                {#if currentStep === 1}
                    ...
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-2xl font-bold text-text">Welcome to Safe Agent</h2>
                        <p class="text-text-muted mt-2">Let's set up your AI agent. You can change these later in Settings.</p>
                    </div>

                    <label class="block mb-4">
                        <span class="text-xs font-medium text-text-muted uppercase tracking-wide">Agent Name</span>
                        <input
                            type="text"
                    ...
                    <label class="block mb-2">
                        <span class="text-xs font-medium text-text-muted uppercase tracking-wide">Core Personality</span>
                        <textarea
                            ...
                            placeholder="Describe how your agent should behave, e.g. 'You are a helpful assistant that responds concisely.'"
                        ></textarea>
                    </label>
                    <p class="text-xs text-text-muted">This instruction persists across all interactions.</p>
                ...
                {:else if currentStep === 2}
                    ...
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-2xl font-bold text-text">LLM Backend</h2>
                        <p class="text-text-muted mt-2">Choose and test the AI model that powers your agent.</p>
                    </div>

                    <label class="block mb-4">
                        <span class="text-xs font-medium text-text-muted uppercase tracking-wide">Backend</span>
                        <select
                    ...
                    <p class="text-xs text-text-muted mb-4">
                        Make sure the required environment variable for your chosen backend is set
                    ...
                    >
                        {#if llmTesting}
                            <i class="fa-solid fa-spinner fa-spin mr-1"></i> Testing connection...
                        {:else}
                            <i class="fa-solid fa-flask mr-1"></i> Test Connection
                        {/if}
                    </button>
                    ...
                    {#if llmTestOk !== null}
                        <div class="mt-4 p-3 rounded-md text-sm {llmTestOk ? 'bg-green-900/30 border border-green-700/50 text-green-300' : 'bg-red-900/30 border border-red-700/50 text-red-300'}">
                            {#if llmTestOk}
                                <i class="fa-solid fa-check-circle mr-1"></i> <strong>Success!</strong>
                                <p class="mt-1 text-xs opacity-80">{llmTestResult}</p>
                            {:else}
                                <i class="fa-solid fa-times-circle mr-1"></i> <strong>Failed</strong>
                                <p class="mt-1 text-xs opacity-80">{llmTestResult}</p>
                            {/if}
                        </div>
                    {/if}
                ...
                {:else if currentStep === 3}
                    ...
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-2xl font-bold text-text">Messaging Platforms</h2>
                        <p class="text-text-muted mt-2">Optionally connect messaging platforms. You can do this later in Settings.</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between p-4 rounded-lg border border-border bg-surface-elevated">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-text">Telegram</p>
                                    <p class="text-xs text-text-muted">Requires <code class="bg-surface px-1 rounded">TELEGRAM_BOT_TOKEN</code></p>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-between p-4 rounded-lg border border-border bg-surface-elevated">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-text-muted">Requires WhatsApp bridge setup</p>
                                </div>
                            </div>
                            <span class="px-2 py-1 text-xs rounded-full font-medium
                                ...
                                {whatsappEnabled ? 'Enabled' : 'Not configured'}
                            </span>
                        </div>
                    </div>

                    <p class="text-xs text-text-muted mt-4">
                        Messaging platforms can be configured later via the config file or environment variables.
                    </p>
                ...
                {:else if currentStep === 4}
                    ...
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-2xl font-bold text-text">Ready to Go</h2>
                        <p class="text-text-muted mt-2">Review your configuration and complete setup.</p>
                    </div>

                    <div class="space-y-3 mb-6">
                        <div class="flex justify-between items-center p-3 rounded-lg bg-surface-elevated border border-border">
                            <span class="text-sm text-text-muted">Agent Name</span>
                            <span class="text-sm font-medium text-text">{agentName || '(default)'}</span>
                        </div>
                        <div class="flex justify-between items-center p-3 rounded-lg bg-surface-elevated border border-border">
                            <span class="text-sm text-text-muted">LLM Backend</span>
                            <span class="text-sm font-medium text-text">{activeBackend || '(default)'}</span>
                        </div>
                        <div class="flex justify-between items-center p-3 rounded-lg bg-surface-elevated border border-border">
                            <span class="text-sm text-text-muted">Telegram</span>
                            <span class="text-sm font-medium {telegramEnabled ? 'text-green-400' : 'text-text-muted'}">
                                {telegramEnabled ? 'Enabled' : 'Not configured'}
                            </span>
                        </div>
                        <div class="flex justify-between items-center p-3 rounded-lg bg-surface-elevated border border-border">
                            <span class="text-sm text-text-muted">WhatsApp</span>
                        ...
                        {#if corePersonality}
                            <div class="p-3 rounded-lg bg-surface-elevated border border-border">
                                <span class="text-xs text-text-muted uppercase tracking-wide">Personality</span>
                    ...
                    <p class="text-xs text-text-muted text-center mb-2">
                        You can change all of these later from the Settings tab.
                    </p>
                {/if}
            </div>
            ...
            <div class="flex justify-between items-center px-8 py-5 bg-surface-elevated border-t border-border">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs text-text-muted">Step {currentStep} of {totalSteps}</span>
