        ...
        {#if stats}
            <div class="grid grid-cols-2 gap-3">
                <div class="text-center">
                    ...
                    <div class="text-[11px] text-text-muted uppercase tracking-wider mt-0.5">Ticks</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-primary-500">{stats.total_actions}</div>
                    <div class="text-[11px] text-text-muted uppercase tracking-wider mt-0.5">Actions</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-success-500">{stats.total_approved}</div>
                    <div class="text-[11px] text-text-muted uppercase tracking-wider mt-0.5">Approved</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-error-500">{stats.total_rejected}</div>
                    <div class="text-[11px] text-text-muted uppercase tracking-wider mt-0.5">Rejected</div>
                </div>
            </div>
            <div class="mt-3 text-xs text-text-muted">
                {#if stats.last_tick_at}
                    <i class="fa-regular fa-clock mr-1"></i>Last tick: {stats.last_tick_at}<br>
                {:else}
                    No ticks yet<br>
                {/if}
